...
contract Token is ERC20, ERC20Burnable, Ownable {
    ...
    function setCrowdsale(address _crowdsale, address _timelock) public onlyOwner {
    ...
    function burnTimelockToken(address account, uint amount) external onlyTimelock {
        require(account != address(0), "Non zere account");
        address owner = owner();
        require(allowance(owner, account) >= amount);
